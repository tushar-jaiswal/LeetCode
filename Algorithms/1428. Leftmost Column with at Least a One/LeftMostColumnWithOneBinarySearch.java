...
class Solution {
    public int leftMostColumnWithOne(BinaryMatrix binaryMatrix) {
        ...
        int earliestCol = numCols;

        for (int i = 0; i < numRows; i++) {
            int low = 0, high = earliestCol - 1;

            while (low <= high) {
                int mid = low + (high - low) / 2;

                int item = binaryMatrix.get(i, mid);
                if (item == 0) {
                    low = mid + 1;
                } else {
                    high = mid - 1;
                    earliestCol = Math.min(earliestCol, mid);
                }
            }
        }

        return earliestCol == numCols ? -1 : earliestCol;
    }
}
