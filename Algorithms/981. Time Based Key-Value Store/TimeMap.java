...
/*Runtime Complexity: set is O(1) and get is O(log(n)) as in the worst case all TimeMap entries are for the same key
...
class TimeMap {
    ...
    private String binarySearch(List<Pair<Integer, String>> list, int target) {
        ...
        while (low <= high) {
            ...
            } else if (pair.getKey() < target && (mid + 1 < list.size() && list.get(mid + 1).getKey() > target) || (mid + 1 == list.size())) {
                return pair.getValue();
