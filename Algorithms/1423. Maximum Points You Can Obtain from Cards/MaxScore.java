...
class Solution {
    public int maxScore(int[] cardPoints, int k) {
        if (k > cardPoints.length) {
            throw new IllegalArgumentException("k must not be greater than the length of the cardPoints array.");
        }
        ...
        for (int i = 0; i < cardPoints.length; i++) {
            ...
            if (i < minSubArrayLength) {
                minSubArraySum = currSubArraySum;
            } else {
                currSubArraySum -= cardPoints[i - minSubArrayLength];
