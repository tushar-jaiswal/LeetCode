...
class SQL {
    ...
    public SQL(List<String> names, List<Integer> columns) {
        if (names.size() != columns.size()) {
            throw new IllegalArgumentException("Number of tables and columns supplied for each doesn't match");
        }

        this.tables = new HashMap<>();
        for (int i = 0; i < names.size(); i++) {
            this.tables.put(names.get(i), new Table(names.get(i), columns.get(i)));
        }
    }

    public boolean ins(String name, List<String> row) {
        if (!tables.containsKey(name) || tables.get(name).columnCount != row.size()) {
            return false;
        }

        Table table = tables.get(name);
        int rowId = table.nextId;
        table.rows.put(rowId, new ArrayList<>(row));
        table.nextId++;
        return true;
    }
    ...
    public String sel(String name, int rowId, int columnId) {
        if (!tables.containsKey(name) ||!tables.get(name).rows.containsKey(rowId) ||
            columnId > tables.get(name).columnCount || columnId <= 0) {
            return "<null>";
        }

        return tables.get(name).rows.get(rowId).get(columnId - 1);
    }
    ...
    public List<String> exp(String name) {
        ...
        for (Map.Entry<Integer, List<String>> entry : table.rows.entrySet()) {
            ...
            result.add(rowId + "," + String.join(",", row));
        }
